...
- Instantiate your `SessionStorageBase`-based class, passing a partition key that makes sense in your application. This could be a user name, ASP.NET SessionId or whatever makes sense in your application. (I'm not clear that a SessionId makes sense -- reall I envisioned always using a user name as the partition key.)
